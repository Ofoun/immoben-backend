...
function deleteState() {
	...
	$.ajax({
		...
		changeFormStateToNew();
		showToastMessage("L'état a été supprimé");
	}).fail(function() {
		showToastMessage("ERREUR : impossible de se connecter au serveur ou le serveur a rencontré une erreur");
	});		
}
...
function loadStates4Country() {
	...
	$.get(url, function(responseJSON) {
		...
		changeFormStateToNew();
		showToastMessage("Tous les états ont été chargés pour le pays " + selectedCountry.text());
	}).fail(function() {
		showToastMessage("ERREUR : impossible de se connecter au serveur ou le serveur a rencontré une erreur");
	});	
}
...
function loadCountries4States() {
	...
	$.get(url, function(responseJSON) {
		...
		showToastMessage("Tous les pays ont été téléchargés");
	}).fail(function() {
		showToastMessage("ERREUR : impossible de se connecter au serveur ou le serveur a rencontré une erreur");
	});
}